...
export function registerProjectTools(server, callKicadScript) {
    // Create project tool
    server.tool("create_project", "Create a new KiCAD project", {
    ...
    // Open project tool
    server.tool("open_project", "Open an existing KiCAD project", {
    ...
    // Save project tool
    server.tool("save_project", "Save the current KiCAD project", {
    ...
    server.tool("get_project_info", "Get information about the current KiCAD project", {}, async () => {
    ...
    server.tool("snapshot_project", "Save a named checkpoint snapshot of the current project state (renders board to PDF and records step label). Call after completing each major step — e.g. after Step 1 (schematic_ok) and Step 2 (layout_ok). Required by the demo workflow before waiting for user confirmation.", {
        ...
        prompt: z
            .string()
            .optional()
            .describe("Full prompt text to save as PROMPT_step{step}_{timestamp}.md alongside the snapshot"),
    }, async (args) => {
