...
export function registerSchematicTools(server, callKicadScript) {
    ...
    server.tool("create_schematic", "Create a new schematic", {
    ...
    server.tool("add_schematic_component", "Add a component to the schematic. Symbol format is 'Library:SymbolName' (e.g., 'Device:R', 'EDA-MCP:ESP32-C3')", {
    ...
    server.tool("delete_schematic_component", `Remove a placed symbol from a KiCAD schematic (.kicad_sch).
...
To remove a footprint from a PCB, use delete_component instead.`, {
    ...
    server.tool("edit_schematic_component", `Update properties of a placed symbol in a KiCAD schematic (.kicad_sch) in-place.
...
Note: operates on .kicad_sch files only. To modify a PCB footprint use edit_component.`, {
        ...
        fieldPositions: z
            ...
            .optional()
            .describe('Reposition field labels: map of field name to {x, y, angle} (e.g. {"Reference": {"x": 12.5, "y": 17.0}})'),
    }, async (args) => {
    ...
    server.tool("get_schematic_component", "Get full component info from a schematic: position, field values, and each field's label position (at x/y/angle). Use this to inspect or prepare repositioning of Reference/Value labels.", {
        schematicPath: z.string().describe("Path to the .kicad_sch file"),
        ...
        if (result.success) {
            const pos = result.position
                ...
                : "unknown";
            const fieldLines = Object.entries(result.fields ?? {}).map(([name, f]) => `  ${name}: "${f.value}" @ (${f.x}, ${f.y}, angle=${f.angle}°)`);
            return {
                content: [
                    {
                        type: "text",
    ...
    server.tool("add_schematic_junction", "Place a junction dot at a wire intersection in the schematic. Required at T-branch and X-cross points so KiCAD recognises the electrical connection.", {
        schematicPath: z.string().describe("Path to the .kicad_sch file"),
    ...
    // Add net label
    server.tool("add_schematic_net_label", "Add a net label to the schematic. " +
        ...
        position: z
            ...
            .length(2)
            .optional()
            .describe("Position [x, y] for the label. Required when componentRef/pinNumber are not given."),
        componentRef: z
            .string()
            .optional()
        ...
        pinNumber: z
            ...
            .optional()
            .describe("Pin number or name on componentRef to snap label to (e.g. '1', 'GND'). Use with componentRef."),
        labelType: z
    ...
    // Connect pin to net
    server.tool("connect_to_net", "Connect a component pin to a named net by adding a wire stub and net label at the exact pin endpoint. " +
    ...
    // Get net connections
    server.tool("get_net_connections", "Get all connections for a named net", {
        schematicPath: z.string().describe("Path to the schematic file"),
        ...
        if (result.success && result.connections) {
            const connectionList = result.connections
                .map((conn) => `  - ${conn.component}/${conn.pin}`)
                .join("\n");
            return {
                content: [
                    {
    ...
    server.tool("get_wire_connections", "Returns the net name and all wires and component pins connected at a given point. " +
    ...
    server.tool("get_schematic_pin_locations", "Returns the exact x/y coordinates of every pin on a schematic component. Use this before add_schematic_net_label to place labels correctly on pin endpoints.", {
        schematicPath: z.string().describe("Path to the schematic file"),
        ...
        if (result.success && result.pins) {
            const lines = Object.entries(result.pins).map(([pinNum, data]) => `  Pin ${pinNum} (${data.name || pinNum}): x=${data.x}, y=${data.y}, angle=${data.angle ?? 0}°`);
            return {
                content: [
                    {
                        type: "text",
    ...
    server.tool("connect_passthrough", "Connects all pins of a source connector (e.g. J1) to matching pins of a target connector (e.g. J2) via shared net labels — pin N gets net '{netPrefix}_{N}'. Use this for FFC/ribbon cable passthrough adapters instead of calling connect_to_net for every pin.", {
        schematicPath: z.string().describe("Path to the schematic file"),
        ...
        if (result.success !== false || (result.connected && result.connected.length > 0)) {
            ...
            if (result.connected?.length)
                lines.push(`Connected (${result.connected.length}): ${result.connected.slice(0, 5).join(", ")}${result.connected.length > 5 ? " ..." : ""}`);
    ...
    server.tool("list_schematic_components", "List all components in a schematic with their references, values, positions, and pins. Essential for inspecting what's on the schematic before making edits.", {
        schematicPath: z.string().describe("Path to the .kicad_sch file"),
        filter: z
            .object({
        ...
        if (result.success) {
            ...
            const lines = comps.map((c) => `  ${c.reference}: ${c.libId} = "${c.value}" at (${c.position.x}, ${c.position.y}) rot=${c.rotation}°${c.pins ? ` [${c.pins.length} pins]` : ""}`);
            return {
                content: [
                    {
                        type: "text",
    ...
    server.tool("list_schematic_nets", "List all nets in the schematic with their connections.", {
        schematicPath: z.string().describe("Path to the .kicad_sch file"),
    }, async (args) => {
        ...
        if (result.success) {
            ...
            const lines = nets.map((n) => {
                const conns = (n.connections || []).map((c) => `${c.component}/${c.pin}`).join(", ");
                const pinCount = n.connected_pin_count !== undefined ? ` [${n.connected_pin_count} pin(s)]` : "";
    ...
    server.tool("list_schematic_wires", "List all wires in the schematic with start/end coordinates.", {
        schematicPath: z.string().describe("Path to the .kicad_sch file"),
    }, async (args) => {
        ...
        if (result.success) {
            ...
            const lines = wires.map((w) => `  (${w.start.x}, ${w.start.y}) → (${w.end.x}, ${w.end.y})`);
            return {
                content: [
                    {
                        type: "text",
    ...
    server.tool("list_schematic_labels", "List all net labels, global labels, and power flags in the schematic.", {
        schematicPath: z.string().describe("Path to the .kicad_sch file"),
    }, async (args) => {
        ...
        if (result.success) {
            ...
            const lines = labels.map((l) => `  [${l.type}] ${l.name} at (${l.position.x}, ${l.position.y})`);
            return {
                content: [
                    {
                        type: "text",
    ...
    server.tool("move_schematic_component", "Move a placed symbol to a new position in the schematic. By default (preserveWires=true) wire endpoints touching the component's pins are stretched to follow the new position.", {
        schematicPath: z.string().describe("Path to the .kicad_sch file"),
        ...
        if (result.success) {
            ...
            return {
                content: [
                    {
                        type: "text",
                        text: `Moved ${args.reference} from (${result.oldPosition.x}, ${result.oldPosition.y}) ` +
    ...
    server.tool("rotate_schematic_component", "Rotate a placed symbol in the schematic.", {
        schematicPath: z.string().describe("Path to the .kicad_sch file"),
        reference: z.string().describe("Reference designator (e.g., R1, U1)"),
    ...
    // Annotate schematic
    server.tool("annotate_schematic", "Assign reference designators to unannotated components (R? → R1, R2, ...). Must be called before tools that require known references.", {
        schematicPath: z.string().describe("Path to the .kicad_sch file"),
    }, async (args) => {
        ...
        if (result.success) {
            ...
            const lines = annotated.map((a) => `  ${a.oldReference} → ${a.newReference}`);
            return {
                content: [
                    {
                        type: "text",
    ...
    server.tool("delete_schematic_wire", "Remove a wire from the schematic by start and end coordinates.", {
        schematicPath: z.string().describe("Path to the .kicad_sch file"),
    ...
    server.tool("delete_schematic_net_label", "Remove a net label from the schematic.", {
        schematicPath: z.string().describe("Path to the .kicad_sch file"),
    ...
    server.tool("export_schematic_svg", "Export schematic to SVG format using kicad-cli.", {
        schematicPath: z.string().describe("Path to the .kicad_sch file"),
    ...
    server.tool("export_schematic_pdf", "Export schematic to PDF format using kicad-cli.", {
        schematicPath: z.string().describe("Path to the .kicad_sch file"),
    ...
    server.tool("get_schematic_view", "Return a rasterized image of the schematic (PNG by default, or SVG). Uses kicad-cli to export SVG, then converts to PNG via cairosvg. Use this for visual feedback after placing or wiring components.", {
        schematicPath: z.string().describe("Path to the .kicad_sch file"),
    ...
    server.tool("run_erc", "Runs the KiCAD Electrical Rules Check (ERC) on a schematic and returns all violations. Use after wiring to verify the schematic before generating a netlist.", {
        schematicPath: z.string().describe("Path to the .kicad_sch schematic file"),
    }, async (args) => {
        const result = await callKicadScript("run_erc", args);
        if (result.success) {
            const violations = result.violations || [];
            const lines = [`ERC result: ${violations.length} violation(s)`];
            ...
            if (violations.length > 0) {
                ...
                violations.slice(0, 30).forEach((v, i) => {
                    const loc = v.location && v.location.x !== undefined
    ...
    server.tool("generate_netlist", "Return a structured JSON netlist from the schematic — component list (reference, value, footprint) and net list (net name with all connected component/pin pairs). Use this to inspect or verify connectivity within the conversation. Does not write any file. To export a netlist file in Spice, KiCad XML, Cadstar, or OrcadPCB2 format, use export_netlist instead.", {
        ...
        if (result.success && result.netlist) {
            ...
            const output = [
                ...
                ...netlist.components.map((comp) => `  ${comp.reference}: ${comp.value} (${comp.footprint || "No footprint"})`),
    ...
    server.tool("get_schematic_view_region", "Export a cropped region of the schematic as an image (PNG or SVG). Specify bounding box coordinates in schematic mm. Useful for zooming into a specific area to inspect wiring or layout.", {
        schematicPath: z.string().describe("Path to the .kicad_sch schematic file"),
    ...
    server.tool("find_overlapping_elements", "Detect spatially overlapping symbols, wires, and labels in the schematic. Finds duplicate power symbols at the same position, collinear overlapping wires, and labels stacked on top of each other.", {
        schematicPath: z.string().describe("Path to the .kicad_sch schematic file"),
        tolerance: z
            .number()
            .optional()
            .describe("Distance threshold in mm for label proximity and wire collinearity checks. Symbol overlap uses bounding-box intersection. (default: 0.5)"),
    }, async (args) => {
        const result = await callKicadScript("find_overlapping_elements", args);
        if (result.success) {
            const lines = [`Found ${result.totalOverlaps} overlap(s):`];
            const syms = result.overlappingSymbols || [];
            const lbls = result.overlappingLabels || [];
            const wires = result.overlappingWires || [];
            if (syms.length) {
                lines.push(`\nOverlapping symbols (${syms.length}):`);
                syms.slice(0, 20).forEach((o) => {
                    lines.push(`  ${o.element1.reference} ↔ ${o.element2.reference} (${o.distance}mm) [${o.type}]`);
                });
            }
            if (lbls.length) {
                lines.push(`\nOverlapping labels (${lbls.length}):`);
                lbls.slice(0, 20).forEach((o) => {
                    lines.push(`  "${o.element1.name}" ↔ "${o.element2.name}" (${o.distance}mm)`);
                });
            }
            if (wires.length) {
                lines.push(`\nOverlapping wires (${wires.length}):`);
                wires.slice(0, 20).forEach((o) => {
    ...
    server.tool("get_elements_in_region", "List all symbols, wires, and labels within a rectangular region of the schematic. Useful for understanding what is in a specific area before modifying it.", {
        schematicPath: z.string().describe("Path to the .kicad_sch schematic file"),
        x1: z.number().describe("Left X coordinate of the region in mm"),
        y1: z.number().describe("Top Y coordinate of the region in mm"),
        x2: z.number().describe("Right X coordinate of the region in mm"),
    ...
    server.tool("find_wires_crossing_symbols", "Find all wires that cross over component symbol bodies. Wires passing over symbols are unacceptable in schematics — they indicate routing mistakes where a wire was drawn across a component instead of around it.", {
        schematicPath: z.string().describe("Path to the .kicad_sch schematic file"),
    }, async (args) => {
        ...
        if (result.success) {
            ...
            collisions.slice(0, 30).forEach((c, i) => {
                lines.push(`  ${i + 1}. Wire (${c.wire.start.x},${c.wire.start.y})→(${c.wire.end.x},${c.wire.end.y}) crosses ${c.component.reference} (${c.component.libId})`);
            });
    ...
    server.tool("list_floating_labels", "Returns all net labels in the schematic that are not connected to any component pin. " +
        ...
        if (result.success) {
            const labels = result.floating_labels || [];
            if (labels.length === 0) {
                return { content: [{ type: "text", text: "No floating labels found." }] };
            }
            const lines = [`Found ${labels.length} floating label(s):\n`];
            labels.slice(0, 50).forEach((lbl) => {
    ...
    // Find orphaned wires
    server.tool("find_orphaned_wires", "Find wire segments with at least one dangling endpoint — not connected to a component pin, " +
        ...
        if (result.success) {
            const wires = result.orphaned_wires || [];
            if (wires.length === 0) {
                return { content: [{ type: "text", text: "No orphaned wires found." }] };
            }
            const lines = [`Found ${wires.length} orphaned wire(s):\n`];
            wires.slice(0, 50).forEach((w) => {
                const dangling = w.dangling_ends.map((e) => `(${e.x}, ${e.y})`).join(", ");
                lines.push(`  wire (${w.start.x}, ${w.start.y})→(${w.end.x}, ${w.end.y})  dangling end(s): ${dangling}`);
            });
            if (wires.length > 50)
                lines.push(`  ... and ${wires.length - 50} more`);
            return { content: [{ type: "text", text: lines.join("\n") }] };
        }
        return {
            content: [{ type: "text", text: `Failed: ${result.message || "Unknown error"}` }],
    ...
    server.tool("snap_to_grid", "Snap schematic element coordinates to the nearest grid point. " +
        ...
        elements: z
            ...
            .optional()
            .describe('Element types to snap (default: ["wires", "junctions", "labels"]). ' +
    ...
    server.tool("get_net_at_point", "Returns the net name at a given (x, y) coordinate in a schematic, or null if no net label " +
        ...
        if (result.success) {
            ...
            return {
                content: [
                    {
                        type: "text",
                        text: `Net at (${pos?.x ?? args.x}, ${pos?.y ?? args.y}): ` +
