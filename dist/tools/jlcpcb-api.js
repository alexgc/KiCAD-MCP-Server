 ...
 */
import { z } from "zod";
export function registerJLCPCBApiTools(server, callKicadScript) {
    // Download JLCPCB parts database
    server.tool("download_jlcpcb_database", `Download the complete JLCPCB parts catalog to local database.
...
for fast offline searching.`, {
    ...
    // Search JLCPCB parts
    server.tool("search_jlcpcb_parts", `Search JLCPCB parts catalog by specifications.
...
Use this to find components with exact specifications and cost optimization.`, {
    ...
    server.tool("get_jlcpcb_part", "Get detailed information about a specific JLCPCB part by LCSC number", {
    ...
    server.tool("get_jlcpcb_database_stats", "Get statistics about the local JLCPCB parts database", {}, async () => {
    ...
    server.tool("suggest_jlcpcb_alternatives", `Suggest alternative JLCPCB parts for a given component.
...
//# sourceMappingURL=jlcpcb-api.js.map