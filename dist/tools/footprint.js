...
// ---- tool registration --------------------------------------------------- //
export function registerFootprintTools(server, callKicadScript) {
    // ── create_footprint ──────────────────────────────────────────────────── //
    server.tool("create_footprint", "Create a new KiCAD footprint (.kicad_mod) inside a .pretty library directory. " +
        ...
        courtyard: RectSchema.optional().describe("Courtyard rectangle on F.CrtYd (recommended: 0.25 mm clearance around pads)"),
        silkscreen: RectSchema.optional().describe("Silkscreen rectangle on F.SilkS"),
        fabLayer: RectSchema.optional().describe("Fab-layer rectangle on F.Fab (shows component body)"),
        refPosition: z
    ...
    // ── edit_footprint_pad ────────────────────────────────────────────────── //
    server.tool("edit_footprint_pad", "Edit an existing pad inside a .kicad_mod footprint file. " +
    ...
    server.tool("register_footprint_library", "Register a .pretty footprint library in KiCAD's fp-lib-table so KiCAD can find the footprints. " +
        ...
        scope: z
            .enum(["project", "global"])
            .optional()
            .describe("project = writes fp-lib-table next to the .kicad_pro file (default); " +
            "global = writes to the user's global KiCAD config"),
        projectPath: z
            .string()
            .optional()
            .describe("Path to the .kicad_pro file or its directory (required for scope=project " +
    ...
    server.tool("list_footprint_libraries", "List available .pretty footprint libraries and their contents (first 20 footprints per library). " +
        "Searches KiCAD standard install paths by default.", {
        searchPaths: z
            .array(z.string())
            .optional()
            .describe("Override default search paths. Each entry should be a directory that contains .pretty subdirs."),
    }, async (args) => {
