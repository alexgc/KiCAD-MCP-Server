...
const PinSchema = z.object({
    ...
    at: z
        ...
        angle: z
            .number()
            .describe("Direction the pin wire extends FROM the symbol body: 0=right, 90=up, 180=left, 270=down"),
    })
...
export function registerSymbolCreatorTools(server, callKicadScript) {
    // ── create_symbol ────────────────────────────────────────────────────── //
    server.tool("create_symbol", "Create a new schematic symbol in a .kicad_sym library file (created if missing). " +
    ...
    // ── delete_symbol ────────────────────────────────────────────────────── //
    server.tool("delete_symbol", "Remove a symbol from a .kicad_sym library file.", {
    ...
    server.tool("list_symbols_in_library", "List all symbol names in a .kicad_sym library file.", {
        libraryPath: z.string().describe("Path to the .kicad_sym file"),
    }, async (args) => {
    ...
    server.tool("register_symbol_library", "Register a .kicad_sym library in KiCAD's sym-lib-table so symbols can be used in schematics. " +
