...
export function registerLibraryTools(server, callKicadScript) {
    ...
    server.tool("list_libraries", "List all available KiCAD footprint libraries", {
        search_paths: z
            .array(z.string())
            .optional()
    ...
    server.tool("search_footprints", "Search for footprints matching a pattern across all libraries", {
        ...
        if (result.success && result.footprints) {
            const footprintList = result.footprints
                .map((fp) => `${fp.full_name || fp.library + ":" + fp.footprint}${fp.description ? " - " + fp.description : ""}`)
                .join("\n");
            return {
                content: [
                    {
    ...
    server.tool("list_library_footprints", "List all footprints in a specific KiCAD library", {
        ...
        if (result.success && result.footprints) {
            const footprintList = result.footprints.map((fp) => `  - ${fp}`).join("\n");
            return {
                content: [
                    {
                        type: "text",
    ...
    server.tool("get_footprint_info", "Get detailed information about a specific footprint", {
        ...
        if (result.success && result.info) {
            ...
            const padsArray = Array.isArray(info.pads)
                ? info.pads
                : [];
